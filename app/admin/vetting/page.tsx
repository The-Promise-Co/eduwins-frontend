...
function NavTab({ active, onClick, label, icon, count }: NavTabProps): ReactElement {
  return (
    <button
      onClick={onClick}
      className={`flex-1 flex flex-col items-center justify-center py-6 rounded-2xl transition duration-500 relative ${
        active ? 'bg-[#001A72] text-white shadow-2xl scale-[1.02] z-10' : 'text-[#001A72]/40 hover:bg-gray-50'
      }`}
    >
      <span className="text-2xl mb-1">{icon}</span>
      <span className="text-[10px] font-black uppercase tracking-[0.2em]">{label}</span>
      {count !== undefined && (
        <span className={`absolute top-4 right-4 text-[10px] font-black px-2 py-0.5 rounded-lg ${
          active ? 'bg-[#FFB81C] text-[#001A72]' : 'bg-gray-100 text-gray-400'
        }`}>
          {count}
        </span>
      )}
