...
export default function PublicCourseDetailPage() {
  ...
  const handleEnroll = useCallback(async () => {
    ...
    if (!course || isCourseOwner) return;

    try {
      setPayError(null);
      const result = await enrollMutation.mutateAsync({
        email: user?.email,
        callback_url: `${window.location.origin}/courses/payment/confirm`,
      });

      if (result.requiresPayment) {
        const paymentUrl = result.authorizationUrl || result.authorization_url;
        if (!paymentUrl) {
          setPayError('Payment link was not returned. Please try again.');
          return;
        }
      ...
      console.error('Course enrollment error:', err.response?.data || err);
      setPayError(err.response?.data?.error || 'Enrollment failed. Please try again.');
    }
  }, [course, id, isAuthenticated, isCourseOwner, user, enrollMutation, router]);
                ...
                {isCourseOwner && isAuthenticated ? (
                  <div className="space-y-2">
                    <Link
                      ...
                      className="block w-full bg-[#FFB81C] text-[#001A72] py-3 rounded-xl font-black text-sm hover:bg-[#FFB81C]/90 transition shadow-lg text-center"
                    >
                      Manage Course
                    </Link>
                  </div>
                    ...
                    <div className="text-center py-2">
                      ...
                      <Link
                        ...
                        className="block w-full mt-3 bg-[#FFB81C] text-[#001A72] py-2.5 rounded-xl font-black text-sm hover:bg-[#FFB81C]/90 transition shadow-lg text-center"
                      >
                        Start Course
                      </Link>
                    </div>
                  ...
                  <>
                    ...
                    >
                        {enrollMutation.isPending
                          ? course.is_free ? 'Enrolling…' : 'Redirecting…'
                          : course.is_free
                            ? 'Enroll for Free'
                            : 'Enroll Now'}
                    </button>
          ...
          <div className="space-y-4">
            ...
            {course.teacher_name && (
              <div className="bg-gradient-to-br from-[#001A72] to-[#0028a8] rounded-2xl p-5 text-white text-center">
                ...
                {tutorHref ? (
                  <Link href={tutorHref} className="font-bold text-sm hover:text-[#FFB81C] transition">
                    {course.teacher_name}
                  </Link>
                ) : (
