...
function SettingToggle({ label, sub, active, onClick }: SettingToggleProps): ReactElement {
  return (
    <div className="flex items-center justify-between p-6 bg-gray-50/50 rounded-2xl border-2 border-transparent hover:border-gray-100 transition">
      ...
      <button
        onClick={onClick}
        className={`w-14 h-8 rounded-full transition-colors duration-300 relative ${active ? 'bg-[#FFB81C]' : 'bg-gray-200'
          }`}
      >
        <div className={`absolute top-1 w-6 h-6 bg-white rounded-full shadow-sm transition-transform duration-300 ${active ? 'translate-x-7' : 'translate-x-1'
          }`} />
      </button>
    </div>
  );
