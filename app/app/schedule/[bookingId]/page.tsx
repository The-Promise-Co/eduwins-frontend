...
import { useCreateDispute } from '@/misc/hooks/api/disputes';
import { useSavePersonalNotes, useSaveSharedNotes, useSessionNotes, useRetrySnapshotSync } from '@/misc/hooks/api/notes';
...
export default function BookingDetailPage() {
  ...
  const retrySnapshotSync = useRetrySnapshotSync(bookingId);

  const handleRetrySnapshotSync = () => {
    retrySnapshotSync.mutate(undefined, {
      onSuccess: ({ saved, failed }) => {
        if (failed > 0) {
          toast.error(`${saved} snapshot${saved === 1 ? '' : 's'} synced, ${failed} still pending.`);
        } else {
          toast.success(saved > 0 ? `${saved} snapshot${saved === 1 ? '' : 's'} synced.` : 'Snapshots are already in sync.');
        }
      },
      onError: () => {
        toast.error('Sync failed. Please check your connection and retry.');
      },
    });
  };
  ...
  const createDispute = useCreateDispute(bookingId);
  ...
  // Remount = full reload from the DB (source of truth).
  useEffect(() => {
    setPersonalIndex(0);
    setSharedIndex(0);
  ...
  const handleAddNote = () => {
    ...
    if (isShared) {
      void saveSharedNotes.mutateAsync(updated).then(
        () => toast.success(`Created ${newNote.title}`),
        () => toast.error('Notes not saved — server save failed.', {
          action: { label: 'Retry', onClick: () => void saveSharedNotes.mutateAsync(updated).catch(() => undefined) },
        }),
      );
      setSharedIndex(updated.length - 1);
    } else {
      void savePersonalNotes.mutateAsync(updated).then(
        () => toast.success(`Created ${newNote.title}`),
        () => toast.error('Notes not saved — server save failed.', {
          action: { label: 'Retry', onClick: () => void savePersonalNotes.mutateAsync(updated).catch(() => undefined) },
        }),
      );
  ...
  const handleSaveNoteEdit = () => {
    ...
    const saveMutation = isShared ? saveSharedNotes : savePersonalNotes;
    void saveMutation.mutateAsync(updatedList).then(
      () => {
        setIsEditingNote(false);
        toast.success('Note saved');
      },
      () => toast.error('Notes not saved — server save failed.', {
        action: { label: 'Retry', onClick: () => void saveMutation.mutateAsync(updatedList).then(() => setIsEditingNote(false)).catch(() => undefined) },
      }),
    );
  };
  ...
  const handleDownloadSnapshot = async (snap: WhiteboardSnapshotItem) => {
    const slug = snap.title.toLowerCase().replace(/\s+/g, '-');
    // Prefer the PNG preview; fall back to the scene JSON when no preview exists.
    if (snap.imageUrl) {
      try {
        const res = await fetch(snap.imageUrl);
        ...
        // fall through to scene download
      }
    }
    if (snap.scene) {
      const blob = new Blob([snap.scene], { type: 'application/json;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `${slug}.excalidraw.json`;
      link.click();
      URL.revokeObjectURL(url);
      toast.success('Whiteboard scene downloaded');
      return;
    }
    toast.error('No snapshot data available to download');
  };
  ...
  // Parents may raise a dispute once the session window opens (15 min before
  // start) until 1 hour after the scheduled end — the same window the
  // settlement cron honors before splitting payment.
  const DISPUTE_WINDOW_MS = 60 * 60 * 1000;
  const nowMs = Date.now();
  const canDispute =
    !isTeacher &&
    !disputeRaised &&
    (booking.status === 'accepted' || booking.status === 'paid_escrow' || booking.status === 'completed') &&
    nowMs >= timing.joinWindowOpen.getTime() &&
    nowMs <= timing.scheduledEnd.getTime() + DISPUTE_WINDOW_MS;

  const handleSubmitDispute = () => {
    const issue = disputeIssue.trim();
    if (!issue) {
    ...
    createDispute.mutate(
      { issue },
      {
        onSuccess: () => {
          ...
          toast.success('Dispute raised, our team will review and reach out as soon as we can');
        },
        onError: (err: any) => {
          const status = err.response?.status;
          const message = err.response?.data?.error || 'Could not raise dispute.';
          if (status === 409) {
            setShowDisputeModal(false);
            setDisputeRaised(true);
          }
  ...
  return (
    <div className="space-y-6 pb-12">
        ...
        <div className="flex items-center gap-2">
          {canDispute && (
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-red-200 bg-white text-xs font-bold text-red-600 hover:bg-red-50 transition shadow-sm"
            >
              <AlertCircle size={14} />
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="space-y-6 lg:col-span-3">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm space-y-4">
            ...
            {notesQuery.data?.snapshotSyncError && (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3.5 flex items-center gap-3">
                <AlertCircle size={18} className="text-amber-600 shrink-0" />
                <p className="flex-1 text-[11px] font-semibold text-amber-800">
                  Could not reach the snapshot database. Showing this device&apos;s copies — they will sync automatically.
                </p>
                <button
                  onClick={handleRetrySnapshotSync}
                  disabled={retrySnapshotSync.isPending}
                  className="px-3 py-1.5 rounded-xl bg-amber-600 text-white text-[11px] font-bold hover:bg-amber-700 transition disabled:opacity-50 shrink-0"
                >
                  {retrySnapshotSync.isPending ? 'Retrying…' : 'Retry now'}
                </button>
              </div>
            )}
                    ...
                    >
                      {snap.imageUrl ? (
                        <img
                          src={snap.imageUrl}
                          alt={snap.title}
                          className="max-w-full max-h-full object-contain scale-90 transition group-hover:scale-95"
                        />
                      ) : (
                        <div className="flex flex-col items-center justify-center gap-1.5 text-gray-300">
                          <Camera size={28} />
                          <span className="text-[10px] font-semibold text-gray-400">Scene saved (no preview)</span>
                        </div>
                      )}
                    ...
                    <div className="p-3.5 flex items-center justify-between gap-2 bg-white">
                      <div>
                        <div className="flex items-center gap-1.5">
                          ...
                          {snap.pending && (
                            <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 border border-amber-200 shrink-0">
                              Saving…
                            </span>
                          )}
                        </div>
          ...
          <div className="w-full min-h-[350px] max-h-[70vh] bg-white rounded-2xl border border-gray-200 p-4 flex items-center justify-center overflow-auto">
            {previewSnapshot.imageUrl ? (
              ...
              <div className="flex flex-col items-center justify-center gap-2 text-gray-400 py-16">
                ...
                <p className="text-xs font-bold">No image preview for this snapshot.</p>
                <p className="text-[11px]">Download the scene JSON to restore it in a whiteboard.</p>
              </div>
            )}
          </div>
        )}
      ...
      <Modal
        ...
        subtitle="Tell us what went wrong with this session. Payment stays on hold while we review."
        size="sm"
        footer={
          <>
            <button
              ...
              className="rounded-xl border border-gray-200 px-4 py-2.5 text-xs font-black uppercase tracking-wider text-gray-500"
            >
              Cancel
            </button>
            <button
              ...
              disabled={createDispute.isPending || !disputeIssue.trim()}
              className="rounded-xl bg-red-600 px-5 py-2.5 text-xs font-black uppercase tracking-wider text-white hover:bg-red-700 transition disabled:opacity-50"
            >
      ...
      >
        <div className="space-y-2">
          ...
          <textarea
            ...
            rows={4}
            maxLength={2000}
            placeholder="e.g. The tutor joined 20 minutes late and ended the session early…"
            className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-2.5 text-xs text-gray-800 outline-none focus:border-red-300 focus:bg-white transition resize-none"
          />
        </div>
      </Modal>
    </div>
