...
export default function SecuritySettingsPage(): ReactElement {
  ...
  useEffect(() => {
    if (user) {
      setSettings((p) => ({ ...p, twoFactorEnabled: user.twoFactorEnabled || false }));
    }
  }, [user]);
  ...
  const toggle2FA = async () => {
    try {
      const res = await toggle2FAMutation.mutateAsync();
      await refreshUser();
      toast.success(res.message || 'Two-factor authentication status updated!');
    } catch (err: any) {
      toast.error(err.response?.data?.error || 'Failed to toggle two-factor authentication.');
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto pb-12">
      <PageHeader
        ...
        subtitle="Manage your active password settings, multi-factor login permissions, and account status"
      />
