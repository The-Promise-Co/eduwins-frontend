...
export default function DigitalVaultPage() {
  ...
  const handlePurchase = async (itemId) => {
    try {
      ...
      // Refresh items to update any purchase status
      fetchItems()
    } catch (err) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading Digital Vault...</p>
        </div>
      </div>
    )
  }
            ...
            <div key={item.id} className="bg-white rounded-lg shadow hover:shadow-lg transition">
              <div className="p-6">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span className="bg-[#FFB81C] text-[#001A72] px-2 py-1 rounded text-xs font-semibold">
                    {item.content_type}
                  </span>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="text-sm text-gray-600 ml-1">
                        {item.rating_avg?.toFixed(1) || 'N/A'} ({item.total_sessions || 0} sessions)
                      </span>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold text-[#001A72]">₦{item.price}</p>
